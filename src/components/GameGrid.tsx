  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
      {displayGames.map((game) => (
        <div
          key={game.id}
          className="bg-slate-200 rounded-lg overflow-hidden shadow-lg flex flex-col group hover:shadow-blue-500/20 transition-all duration-300"
        >
          {/* Image */}
          <div className="aspect-3/4 relative overflow-hidden">
            ...
            {/* Hover Overlay */}
            <div className="absolute inset-0 bg-slate-100/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center backdrop-blur-xs">
              <Link
                to={game.link}
          ...
          {/* Content */}
          <div className="p-3">
            <h3 className="text-sm font-bold text-primary truncate group-hover:text-blue-400 transition-colors">
              {game.title}
            </h3>
            <div className="flex justify-between items-center mt-2">
              <span className="text-xs text-gray-400 bg-slate-300 px-2 py-0.5 rounded-sm">RPG</span>
              <span className="text-xs text-yellow-500 flex items-center gap-1">★ 4.9</span>
            </div>
          </div>
        </div>
      ))}
