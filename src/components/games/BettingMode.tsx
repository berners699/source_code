          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody>
              {savedModes.map((mode, idx) => (
                <tr key={idx} className="hover:bg-slate-200/30 transition-colors text-primary">
    ...
    <div className="p-4 lg:p-6 space-y-6">
      <div className="relative flex items-center p-4 lg:px-6 border-b border-slate-800 bg-slate-100/50 -mx-4 lg:-mx-6 -mt-4 lg:-mt-6 mb-6">
        <button
          ...
          className="absolute left-4 lg:left-6 flex items-center gap-2 text-slate-400 hover:text-primary transition-colors text-sm font-medium z-10"
        >
          <ArrowLeft size={16} />
          返回列表
        </button>
        <div className="flex-1 text-center">
          <span className="text-primary font-bold text-lg">{activeGame.name} 第3161904期</span>
        </div>
      </div>
      {/* Quick Select Sections */}
      <div className="space-y-4 bg-slate-200/30 p-4 rounded-2xl border border-slate-150">
        {/* Row 1: Categories */}
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-all active:scale-95 ${
                selectedQuickItems.includes(item)
                  ? `bg-linear-to-r ${activeGame.colors.primary} border-transparent text-primary shadow-lg`
        ...
        <div className="flex flex-wrap gap-2 items-center">
          <span className="text-xs text-primary mr-2">尾数:</span>
          {[
            '0尾',
            '1尾',
            '2尾',
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-all active:scale-95 ${
                selectedQuickItems.includes(item)
                  ? `bg-linear-to-r ${activeGame.colors.primary} border-transparent text-primary shadow-lg`
                  : 'bg-slate-200 text-primary border-slate-100'
              }`}
            >
              {item}
            </button>
        ...
        {/* Row 3: Remainders */}
        <div className="flex flex-wrap gap-2 items-center">
          <span className="text-xs text-primary mr-2">余数:</span>
          {[
            '3余0',
            '3余1',
            '3余2',
        ...
        {/* Row 4: Multipliers */}
        <div className="flex flex-wrap gap-2 items-center">
          <span className="text-xs text-primary mr-2">倍数:</span>
          {[0.1, 0.5, 0.8, 1.2, 1.5, 2, 5, 10, 100].map((item) => (
            <button
              key={item}
              onClick={() => applyMultiplier(item)}
              className="px-3 py-1.5 rounded-lg bg-slate-200 text-primary text-xs font-medium border border-slate-100 transition-all active:scale-95"
            >
              {item}倍
            </button>
          ))}
        ...
        {/* Row 5: Amounts */}
        <div className="flex flex-wrap gap-2 items-center">
          <span className="text-xs text-primary mr-2">定额:</span>
          ...
          <div className="flex gap-2 ml-2">
            <input
              ...
              className="bg-slate-100 border border-slate-100 rounded-lg px-3 py-1.5 text-xs text-primary focus:outline-hidden focus:border-blue-500 w-32"
            />
            <button
              onClick={() => {
              ...
              className="px-3 py-1.5 rounded-lg bg-slate-300 hover:bg-slate-400 hover:text-primary text-primary text-xs font-medium border border-slate-100 transition-all"
            >
              定额梭哈
            </button>
          </div>
      ...
      <div className="flex flex-wrap gap-3 items-center justify-between">
        <div className="flex flex-wrap items-center gap-4">
          <div className="flex gap-2">
            <button
              ...
              className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-slate-200 hover:bg-slate-300 text-primary text-sm font-medium transition-all"
            >
          ...
          <div className="h-6 w-px bg-primary hidden lg:block"></div>
          ...
          <div className="flex items-center gap-4">
            <div className="flex gap-2">
              <input
                ...
                readOnly
                className="bg-slate-100 border border-slate-700 rounded-lg px-3 py-1.5 text-xs text-primary focus:outline-hidden focus:border-blue-500 w-32"
              />
              {isParticipation && (
                <button
                  onClick={() => {
        ...
        <div className="flex gap-2">
          <button
            ...
            className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-slate-200 hover:bg-slate-300 text-primary text-sm font-medium border border-slate-700 transition-all"
          >
      ...
      {showLastRoundModal && (
        <div className="fixed inset-0 z-110 flex items-center justify-center p-4 bg-black/80 backdrop-blur-xs animate-in fade-in duration-200">
          <div className="bg-slate-100 rounded-3xl w-full max-w-md p-8 shadow-2xl animate-in zoom-in duration-200 relative">
            <button
              onClick={() => setShowLastRoundModal(false)}
              className="absolute right-6 top-6 text-slate-500 hover:text-primary transition-colors"
            >
              <X size={24} />
            </button>
            ...
            <div className="flex flex-col items-center text-center">
              ...
              <h3 className="text-2xl font-bold text-primary mb-2">上期投注</h3>
              ...
              <div className="w-full space-y-4 mb-8">
                <div className="bg-slate-200/50 border border-slate-700 rounded-2xl p-6">
                  <div className="flex justify-between text-sm mb-4">
                    ...
                    <span className="text-slate-500 font-mono">1, 3, 5, 7, 9, 11, 13</span>
                  </div>
                  <div className="flex justify-between text-sm mb-4">
              ...
              <button
                onClick={() => setShowLastRoundModal(false)}
                className="w-full py-4 rounded-2xl bg-linear-to-r from-blue-500 to-blue-600 text-primary font-bold shadow-lg shadow-blue-500/20 transition-all active:scale-95"
              >
                我知道了
              </button>
            </div>
      ...
      {toast.visible && (
        <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-200 animate-in fade-in zoom-in duration-200">
          <div className="bg-white/95 backdrop-blur-md border border-slate-200 rounded-xl px-6 py-3 shadow-2xl flex items-center gap-3">
            <div className="w-6 h-6 rounded-full bg-orange-500 flex items-center justify-center text-primary text-xs font-bold">
              !
            </div>
      ...
      {showSaveModal && (
        <div className="fixed inset-0 z-150 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs">
          <div className="bg-slate-100 border border-slate-800 rounded-3xl w-full max-w-md p-8 shadow-2xl animate-in fade-in zoom-in duration-200">
            ...
            <h3 className="text-2xl font-bold text-primary mb-6">保存模式</h3>
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="请输入模式名称"
                  className="w-full bg-slate-200 border border-slate-700 rounded-xl px-4 py-3 text-primary focus:outline-hidden focus:border-blue-500 transition-all"
                />
              </div>
            ...
            <div className="flex gap-3 mt-8">
              <button
                onClick={() => setShowSaveModal(false)}
                className="flex-1 py-3 rounded-xl bg-slate-200 hover:bg-slate-300 text-primary font-bold transition-all"
              >
                取消
              </button>
              <button
                ...
                className={`flex-1 py-3 rounded-xl bg-linear-to-r ${activeGame.colors.primary} text-primary font-bold shadow-lg transition-all active:scale-95`}
              >
                保存
              </button>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {[0, 1].map((colIndex) => (
          <div
            key={colIndex}
            className="overflow-hidden rounded-2xl border border-slate-200 bg-slate-100/50"
          >
            <table className="w-full text-left text-sm">
              <thead className="bg-slate-200/50 text-slate-400">
                <tr>
                    ...
                    >
                      <td className="p-3">
                        <div className="flex items-center gap-3">
                          <div
                            className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-primary shadow-xs transition-all duration-300 ${
                      ...
                      <td className="p-3" onClick={(e) => e.stopPropagation()}>
                        <input
                          ...
                          readOnly
                          className={`w-24 bg-slate-200 rounded-lg px-3 py-1.5 text-sm text-primary focus:outline-hidden border transition-all ${
                            highlighted
                              ? `${activeGame.colors.border}`
                              : 'border-slate-300 focus:border-blue-300'
                          }`}
                        />
                      </td>
                      <td className="p-3" onClick={(e) => e.stopPropagation()}>
                        <div className="flex gap-1">
                          {[0.5, 1.5, 2, 10].map((m) => (
                            <button
                              ...
                              className="px-2 py-1 rounded-sm bg-slate-200 text-slate-400 text-[10px] transition-all active:scale-95"
                            >
                              {m}倍
                            </button>
                          ))}
