...
const BettingMode: React.FC<BettingModeProps> = ({
      ...
      <div className="p-4 lg:p-6 space-y-6">
        <div className="flex justify-start mb-6">
          <button 
            ...
            className={`px-6 py-2 rounded-lg bg-gradient-to-r ${activeGame.colors.primary} text-white text-sm font-bold shadow-lg transition-all active:scale-95 flex items-center gap-2`}
          >
            <PlusCircle size={16} />
            添加模式
          </button>
        ...
        <div className="overflow-hidden rounded-2xl border border-slate-800 bg-slate-900/50">
          <table className="w-full text-left text-sm">
            <thead className="bg-slate-800/50 text-slate-400">
              <tr>
                <th className="p-4 font-medium text-center border-r border-slate-800/50">模式名称</th>
                <th className="p-4 font-medium text-center border-r border-slate-800/50">投注数量</th>
                <th className="p-4 font-medium text-center border-r border-slate-800/50">合计金豆</th>
            ...
            <tbody className="divide-y divide-slate-800">
              {savedModes.map((mode, idx) => (
                <tr key={idx} className="hover:bg-slate-800/30 transition-colors text-slate-300">
                  ...
                  <td className="p-4 text-center border-r border-slate-800/30">{mode.count || 0}</td>
                  <td className="p-4 text-center border-r border-slate-800/30">{mode.total?.toLocaleString() || 0}</td>
                  <td className="p-4 text-center border-r border-slate-800/30">
                    <button className="text-blue-400 hover:text-blue-300 transition-colors">查看详情</button>
                  </td>
  ...
  return (
    <div className="p-4 lg:p-6 space-y-6">
      <div className="relative flex items-center p-4 lg:px-6 border-b border-slate-800 bg-slate-900/50 -mx-4 lg:-mx-6 -mt-4 lg:-mt-6 mb-6">
        <button 
      ...
      <div className="space-y-4 bg-slate-800/30 p-4 rounded-2xl border border-slate-700/50">
        ...
        <div className="flex flex-wrap gap-2">
          {['单', '大单', '小单', '单边', '双', '大双', '小双', '双边', '大', '小', '中', '边', '大边', '小边'].map(item => (
            <button 
              key={item} 
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-all active:scale-95 ${
                selectedQuickItems.includes(item)
                  ? `bg-gradient-to-r ${activeGame.colors.primary} border-transparent text-white shadow-lg`
        ...
        <div className="flex flex-wrap gap-2 items-center">
          ...
          {['0尾', '1尾', '2尾', '3尾', '4尾', '5尾', '6尾', '7尾', '8尾', '9尾', '大尾', '小尾'].map(item => (
            <button 
              key={item} 
              onClick={() => toggleQuickItem(item)}
              className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-all active:scale-95 ${
        ...
        <div className="flex flex-wrap gap-2 items-center">
          ...
          {['3余0', '3余1', '3余2', '4余0', '4余1', '4余2', '4余3', '5余0', '5余1', '5余2', '5余3', '5余4'].map(item => (
            <button 
              key={item} 
              onClick={() => toggleQuickItem(item)}
              className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-all active:scale-95 ${
        ...
        <div className="flex flex-wrap gap-2 items-center">
          ...
          {[0.1, 0.5, 0.8, 1.2, 1.5, 2, 5, 10, 100].map(item => (
            <button 
              ...
              className="px-3 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-medium border border-slate-700 transition-all active:scale-95"
            >
              {item}倍
            </button>
          ))}
        ...
        <div className="flex flex-wrap gap-2 items-center">
          ...
          {['5万', '50万', '100万', '500万', '1000万', '2000万', '5000万', '1亿', '2亿'].map(item => (
            <button 
              key={item} 
          ...
          <div className="flex gap-2 ml-2">
            ...
            <button 
              ...
              className="px-3 py-1.5 rounded-lg bg-slate-700 hover:bg-slate-600 text-white text-xs font-medium border border-slate-600 transition-all"
            >
              定额梭哈
            </button>
          </div>
        ...
        <div className="flex flex-wrap items-center gap-4">
          <div className="flex gap-2">
            <button 
              ...
              className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 text-sm font-medium border border-slate-700 transition-all"
            >
              <RotateCcw size={16} /> 反选
            </button>
            <button 
              onClick={() => { clearAll(); setCustomBaseAmount(''); }}
              className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 text-sm font-medium border border-slate-700 transition-all"
            >
              <Trash2 size={16} /> 清空
            </button>
            {!isParticipation && (
              <button 
                onClick={() => {
                  if (totalAmount === 0) { showToast('请填写金额'); return; }
                  if (totalAmount < 500) { showToast('最低500金豆'); return; }
                  setShowSaveModal(true);
                }}
                className={`flex items-center gap-1.5 px-4 py-2 rounded-xl bg-gradient-to-r ${activeGame.colors.primary} text-white text-sm font-bold shadow-lg transition-all active:scale-[0.98]`}
              >
                添加模式
              </button>
            )}
            ...
            <div className="flex gap-2">
              ...
              {isParticipation && (
                <button 
                  ...
                  className={`px-6 py-1.5 rounded-lg bg-gradient-to-r ${activeGame.colors.primary} text-white text-xs font-bold shadow-lg transition-all active:scale-[0.98]`}
                >
                  确定参与
                </button>
              )}
      ...
      {showLastRoundModal && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl w-full max-w-md p-8 shadow-2xl animate-in zoom-in duration-200 relative">
            <button 
            ...
            <div className="flex flex-col items-center text-center">
              ...
              <button 
                onClick={() => setShowLastRoundModal(false)}
                className="w-full py-4 rounded-2xl bg-gradient-to-r from-blue-500 to-blue-600 text-white font-bold shadow-lg shadow-blue-500/20 transition-all active:scale-95"
              >
                我知道了
              </button>
            </div>
      ...
      {showSaveModal && (
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl w-full max-w-md p-8 shadow-2xl animate-in fade-in zoom-in duration-200">
            <button 
            ...
            <div className="space-y-4">
              ...
              <div className="bg-slate-800/50 p-4 rounded-xl border border-slate-700 text-slate-400 text-sm">
            ...
            <div className="flex gap-3 mt-8">
              ...
              <button 
                onClick={() => {
                  if (!newModeName.trim()) { showToast('请输入名称'); return; }
                ...
                className={`flex-1 py-3 rounded-xl bg-gradient-to-r ${activeGame.colors.primary} text-white font-bold shadow-lg transition-all active:scale-95`}
              >
                保存
              </button>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {[0, 1].map(colIndex => (
          <div key={colIndex} className="overflow-hidden rounded-2xl border border-slate-800 bg-slate-900/50">
            <table className="w-full text-left text-sm">
              <thead className="bg-slate-800/50 text-slate-400">
                <tr>
                ...
                {Array.from({ length: 14 }).map((_, i) => {
                  ...
                  return (
                    <tr 
                      ...
                      className={`transition-colors group cursor-pointer ${highlighted ? 'bg-slate-800/50' : 'hover:bg-slate-800/30'}`}
                    >
                      <td className="p-3">
                        <div className="flex items-center gap-3">
                          <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-white shadow-sm transition-all duration-300 ${
                            highlighted 
                              ? `${activeGame.colors.bg} scale-110` 
                              : 'bg-slate-700'
                          }`}>
                            {num}
                          </div>
                          <input 
                            type="checkbox" 
                            checked={highlighted}
                            onChange={(e) => { e.stopPropagation(); toggleNumber(num); }}
                            className={`w-4 h-4 rounded border-slate-700 bg-slate-800 focus:ring-0 transition-colors`}
                            style={{ accentColor: highlighted ? activeGame.colors.hex : undefined }}
                          />
                        </div>
                      </td>
                      <td className={`p-3 font-mono transition-colors ${highlighted ? `${activeGame.colors.accent} font-bold` : 'text-slate-400'}`}>{odds}</td>
                      <td className="p-3" onClick={(e) => e.stopPropagation()}>
                        <input 
                          type="text" 
                          value={betAmounts[num] || 0}
                          readOnly
                          className={`w-24 bg-slate-800 rounded-lg px-3 py-1.5 text-sm text-white focus:outline-none border-2 transition-all ${
                            highlighted 
                        ...
                        <div className="flex gap-1">
                          {[0.5, 1.5, 2, 10].map(m => (
                            <button 
                              ...
                              className="px-2 py-1 rounded bg-slate-800 hover:bg-slate-700 text-slate-400 text-[10px] border border-slate-700 transition-all active:scale-95"
                            >
                              {m}倍
                            </button>
                          ))}
