...
const PastResults: React.FC<PastResultsProps> = ({ upcomingIssues, history, activeGame, onJoinClick }) => {
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-slate-800">
            ...
            {currentPage === 1 && upcomingIssues.map((record, i) => (
              <tr key={`upcoming-${i}`} className="hover:bg-slate-800/30 transition-colors text-slate-300">
                ...
                <td className="p-4">
                  <button 
                    onClick={onJoinClick}
                    className={`px-4 py-1 rounded-lg bg-gradient-to-r ${activeGame.colors.primary} text-white text-xs font-bold shadow-lg transition-all active:scale-95`}
                  >
                    立即参与
                  </button>
                </td>
              ...
              <tr key={i} className="hover:bg-slate-800/30 transition-colors text-slate-300">
                ...
                <td className="p-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`w-6 h-6 rounded-full ${activeGame.colors.bg} flex items-center justify-center text-white text-xs font-bold shadow-sm`}>
                      {record.sum}
                    </span>
                  </div>
                </td>
                ...
                <td className="p-4">
                  <div className="flex flex-col text-xs text-slate-400 gap-1">
                    <span className="flex items-center gap-1">收:{record.income} <GoldBean className="w-3 h-3" /></span>
                    <span className="flex items-center gap-1">竞:{record.invest} <GoldBean className="w-3 h-3" /></span>
                  </div>
                </td>
                <td className="p-4">
                  <span className="px-3 py-1 rounded-full bg-slate-800 text-slate-500 text-xs border border-slate-700">
                    {record.status}
                  </span>
                </td>
              </tr>
        ...
        {Array.from({ length: totalPages }).map((_, i) => (
          <button
            ...
            className={`w-8 h-8 rounded-lg text-sm font-bold transition-all ${
              currentPage === i + 1
                ? `bg-gradient-to-r ${activeGame.colors.primary} text-white shadow-lg scale-110`
