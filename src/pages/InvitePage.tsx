...
import { Copy, Share2, Users, Coins, Gift, CheckCircle2, UserPlus, Megaphone, UserPlus2, Activity, ShieldAlert, Trophy, ChevronRight } from 'lucide-react';
...
const InvitePage = () => {
  ...
  const menuItems = [
    ...
    { id: 'first_deposit', label: '每日首充返利', icon: Coins, color: 'from-orange-500 to-yellow-500' },
    { id: 'turnover', label: '娱乐流水返利', icon: Activity, color: 'from-green-500 to-emerald-500' },
  ...
  const subordinates = [
    { id: 1, userId: "888***001", name: "云游小侠", registerTime: "2023-10-20 14:30", commission: 1200 },
    { id: 2, userId: "888***002", name: "游戏达人", registerTime: "2023-10-21 09:15", commission: 850 },
    { id: 3, userId: "888***003", name: "快乐风男", registerTime: "2023-10-22 18:45", commission: 500 },
    { id: 4, userId: "888***004", name: "至尊宝", registerTime: "2023-10-23 11:20", commission: 2000 },
    { id: 5, userId: "888***005", name: "紫霞仙子", registerTime: "2023-10-24 16:10", commission: 150 },
  ];
  ...
  const renderContent = () => {
    switch (activeMenu) {
      case 'invite':
        return (
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="p-6 border-b border-slate-700/50 relative overflow-hidden group">
                ...
                <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
                  <div className="flex-1 w-full">
                    ...
                    <div className="flex items-center gap-2 bg-slate-900/80 p-2 rounded-xl border border-slate-700 group-hover:border-yellow-500/30 transition-colors">
                      <input 
                        type="text" 
                        value={inviteLink} 
                        readOnly 
                  ...
                  <button 
                    ...
                    className={`
                      ...
                      ${copied 
                        ...
                        : 'bg-gradient-to-r from-yellow-400 via-yellow-500 to-orange-500 text-slate-900 shadow-[0_5px_15px_rgba(234,179,8,0.2)] hover:shadow-[0_10px_20px_rgba(234,179,8,0.3)] hover:-translate-y-0.5'}
                    `}
                  >
              ...
              <div className="p-6 bg-slate-900/30">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-slate-800/50 rounded-xl p-4 border border-slate-700 text-center">
                    <div className="text-slate-500 text-xs mb-2 uppercase tracking-wider">开通会员返利</div>
                    <div className="text-2xl font-black text-orange-500">5%</div>
                  </div>
                  <div className="bg-slate-800/50 rounded-xl p-4 border border-slate-700 text-center">
                    <div className="text-slate-500 text-xs mb-2 uppercase tracking-wider">流水返利</div>
            ...
            {/* Subordinates List */}
            <div className="bg-slate-800/50 backdrop-blur-md rounded-2xl overflow-hidden border border-slate-700 shadow-xl">
              <div className="p-6 border-b border-slate-700 flex flex-col md:flex-row items-center justify-between gap-4 bg-slate-900/30">
                ...
                <div className="flex items-center gap-4">
                  <div className="bg-slate-800/50 border border-slate-700 rounded-xl px-4 py-2 flex items-center gap-3">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left">
                  <thead>
                    <tr className="bg-slate-900/80 text-gray-400 text-[10px] uppercase tracking-widest">
                  ...
                  <tbody className="divide-y divide-slate-700/50 text-gray-300">
                    {subordinates.map((sub) => (
                      <tr key={sub.id} className="hover:bg-white/5 transition-colors group">
                        <td className="px-6 py-4 font-mono text-blue-300 text-base">{sub.userId}</td>
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-2">
                            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-slate-700 to-slate-800 flex items-center justify-center text-xs font-bold border border-slate-600 group-hover:border-blue-500/50 transition-colors">
                              {sub.name[0]}
                            </div>
              ...
              {subordinates.length === 0 && (
                <div className="p-12 text-center">
                  <div className="w-14 h-14 rounded-full bg-slate-800 flex items-center justify-center mx-auto mb-4 text-gray-600">
                    <Users size={28} />
                  </div>
      ...
      case 'rank':
        return (
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="bg-slate-800/50 backdrop-blur-md rounded-2xl p-8 border border-slate-700 shadow-xl relative overflow-hidden">
      ...
      default:
        ...
        return (
          <div className="bg-slate-800/50 backdrop-blur-md rounded-3xl p-20 border border-slate-700 text-center animate-in fade-in slide-in-from-bottom-4 duration-500 relative overflow-hidden">
            <div className={`absolute top-0 left-0 w-full h-1 bg-gradient-to-r ${currentItem?.color}`}></div>
            <div className={`w-32 h-32 rounded-3xl bg-gradient-to-br ${currentItem?.color} flex items-center justify-center mx-auto mb-10 text-white shadow-2xl shadow-current/20 rotate-3 hover:rotate-0 transition-transform duration-500`}>
            ...
            <div className="mt-12 flex justify-center gap-4">
              <div className="px-6 py-3 rounded-2xl bg-slate-900/50 border border-slate-700 text-sm font-bold text-gray-500 uppercase tracking-widest">
                Coming Soon
              </div>
            </div>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-10 relative z-20">
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="w-full lg:w-80 flex-shrink-0">
            <div className="bg-slate-800/80 backdrop-blur-md rounded-3xl border border-slate-700 overflow-hidden sticky top-24 shadow-2xl">
              ...
              <nav className="p-4 space-y-3">
                {menuItems.map((item) => (
                  ...
                  >
                    {activeMenu === item.id && (
                      <div className={`absolute inset-y-0 left-0 w-1.5 bg-gradient-to-b ${item.color}`}></div>
                    )}
                    <div className="flex items-center gap-4">
                      <div className={`
                        w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-300
                        ${activeMenu === item.id 
                          ? `bg-gradient-to-br ${item.color} text-white shadow-lg shadow-current/20 scale-110` 
                          : 'bg-slate-700/50 text-gray-500 group-hover:bg-slate-700 group-hover:text-gray-400'}
                      `}>
                        <item.icon size={20} />
                      </div>
                      <span className={`font-bold transition-colors ${activeMenu === item.id ? 'text-white' : 'group-hover:text-gray-200'}`}>
                        {item.label}
                      </span>
                    </div>
                    <ChevronRight size={18} className={`transition-all duration-300 ${activeMenu === item.id ? 'translate-x-0 opacity-100 text-yellow-500' : '-translate-x-2 opacity-0 group-hover:opacity-50'}`} />
                  </button>
                ))}
              </nav>
            </div>
