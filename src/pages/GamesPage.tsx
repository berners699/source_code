...
import { Clock, Trophy, History, AlertCircle, TrendingUp, Settings, List, PlayCircle, FileText, RotateCcw, Trash2, Layers, PlusCircle, ArrowLeft, X } from 'lucide-react';
    ...
    <div className="min-h-screen bg-slate-950 pb-20">
          ...
          <div className="flex flex-col lg:flex-row gap-8 items-center lg:items-start justify-between">
            ...
            <div className="flex-1 w-full">
              <div className="flex flex-wrap justify-center text-sm text-slate-400 mb-6 gap-8">
                <div className="flex items-center gap-2">
                  <span className={`font-bold ${activeGame.colors.accent}`}>【{activeGame.name}】</span>
                  <span>第 <span className="text-white font-mono">3161903</span> 期 结果</span>
                </div>
              ...
              <div className="flex items-center justify-center gap-6 mb-8">
                {[0, 4, 6].map((num, i) => (
                  <React.Fragment key={i}>
                    <div className={`w-14 h-14 rounded-full flex items-center justify-center text-2xl font-bold text-yellow-300 shadow-lg transform hover:scale-110 transition-transform duration-200
            ...
            <div className="flex-shrink-0 w-full lg:w-auto flex flex-col items-center lg:items-end">
              <div className="text-slate-400 mb-4 text-sm">
                距离 第 <span className={`font-mono font-bold text-lg ${activeGame.colors.accent}`}>3161904</span> 期 开奖剩
              </div>
              
              <div className="flex gap-2 mb-4">
                {['0','0',':','0','0',':','0','9'].map((char, i) => (
                  char === ':' ? (
                    <div key={i} className="text-2xl font-bold text-orange-500/50 flex items-center pb-1 animate-pulse">:</div>
                  ) : (
                    <div key={i} className="w-10 h-12 bg-slate-800 border border-orange-500/50 rounded-lg flex items-center justify-center text-2xl font-bold text-orange-500 shadow-[0_0_15px_rgba(249,115,22,0.2)] backdrop-blur-sm">
                      {char}
                    </div>
                  )
                ))}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="bg-slate-900 rounded-2xl border border-slate-800 overflow-hidden shadow-xl min-h-[400px]">
          ...
          {/* Betting Records */}
          {activeTab === 'records' && (
            <BettingRecords activeGame={activeGame} />
          )}
          ...
          {/* Game Rules */}
          {activeTab === 'rules' && (
            <GameRules activeGame={activeGame} />
          )}
