...
const RankPage = () => {
  ...
  const currentData = allRankData.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);
    ...
    <div className="min-h-screen bg-slate-950 pb-20">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 -mt-10 relative z-20">
        ...
        <div className="flex space-x-4 mb-6">
          ...
          <button
            onClick={() => { setActiveTab('yesterday'); setCurrentPage(1); }}
            className={`flex-1 py-4 rounded-xl font-bold text-lg transition-all shadow-lg ${
              activeTab === 'yesterday'
        ...
        {/* Table */}
        <div className="bg-slate-800/80 backdrop-blur-md rounded-2xl overflow-hidden border border-slate-700 shadow-2xl mb-6">
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead>
                <tr className="bg-slate-900/50 text-gray-400 border-b border-slate-700">
              ...
              <tbody className="divide-y divide-slate-700/50">
                {currentData.map((item) => (
                  <tr key={item.rank} className="hover:bg-slate-700/30 transition-colors">
                    <td className="px-6 py-4 text-center">
                      {item.rank === 1 && <img src="https://emojicdn.elk.sh/🥇" className="w-8 h-8 mx-auto" alt="1st" />}
                      {item.rank === 2 && <img src="https://emojicdn.elk.sh/🥈" className="w-8 h-8 mx-auto" alt="2nd" />}
                      {item.rank === 3 && <img src="https://emojicdn.elk.sh/🥉" className="w-8 h-8 mx-auto" alt="3rd" />}
                      {item.rank > 3 && <span className="text-gray-400 font-bold text-lg">{item.rank}</span>}
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-full bg-slate-700 overflow-hidden border-2 border-yellow-500/50 shadow-[0_0_10px_rgba(234,179,8,0.3)]">
        ...
        <div className="flex items-center justify-center gap-4 py-4">
          <button 
            ...
            className={`p-2 rounded-lg border border-slate-700 transition-all ${currentPage === 1 ? 'text-slate-600 cursor-not-allowed' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}
          >
            <ChevronLeft size={20} />
          </button>
          
          <div className="flex items-center gap-2 text-slate-400 font-medium">
            <div className="bg-slate-800 border border-slate-700 px-4 py-1 rounded-lg text-white">
              {currentPage}
            </div>
            <span>/</span>
          ...
          <button 
            ...
            className={`p-2 rounded-lg border border-slate-700 transition-all ${currentPage === totalPages ? 'text-slate-600 cursor-not-allowed' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}
          >
