...
const VIPPage = () => {
  ...
  return (
    <div className="pt-20 pb-12 min-h-screen bg-slate-950">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-12">
          {benefits.map((plan) => (
            <div 
              ...
              className={`relative bg-gradient-to-br ${plan.bgGradient} rounded-2xl border p-4 overflow-hidden group hover:transform hover:-translate-y-1 transition-all duration-300 shadow-xl cursor-pointer ${
            ...
            >
              ...
              <div className="relative z-10 flex flex-col h-full">
                <div className="flex justify-between items-start mb-4">
                  ...
                  <div className={`p-1.5 rounded-lg bg-white/5 border border-white/10 ${plan.color}`}>
                    <Crown size={16} />
                  </div>
                </div>

                <div className="bg-white/5 rounded-xl p-2 mb-4 border border-white/5">
                  <div className="text-[8px] text-slate-500 uppercase tracking-widest mb-1">赠送</div>
                ...
                <button 
                  className="w-full py-2 rounded-xl text-xs font-bold transition-all active:scale-95 bg-white/10 hover:bg-white/20 text-white border border-white/10"
                >
                  立即开通
                </button>
              </div>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-3xl w-full max-w-md p-8 shadow-2xl animate-in zoom-in duration-200">
              ...
              <div className="flex flex-col items-center text-center">
                ...
                <div className="w-full bg-slate-800/50 border border-slate-700 rounded-2xl p-6 mb-8">
                  <div className="text-sm text-slate-500 mb-1 uppercase tracking-widest">客服 QQ</div>
                  <div className="text-3xl font-black text-blue-400 font-mono tracking-tighter">88888888</div>
                </div>
            ...
            <table className="w-full text-left text-[11px]">
              <thead className="bg-slate-800/50 text-slate-500">
                <tr>
                  ...
                  {benefits.map(b => (
                    <th key={b.level} className={`p-3 font-bold ${b.color}`}>{b.name}</th>
                  ))}
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-800">
                <tr className="hover:bg-white/5 transition-colors">
                  ...
                  {benefits.map(b => <td key={b.level} className="p-3 text-white font-mono">{b.price}</td>)}
                </tr>
                <tr className="hover:bg-white/5 transition-colors">
                  <td className="p-3 text-slate-400">开通赠送</td>
                  {benefits.map(b => <td key={b.level} className="p-3 text-yellow-500 font-bold">{b.reward}</td>)}
                </tr>
                <tr className="hover:bg-white/5 transition-colors">
                  <td className="p-3 text-slate-400">有效期</td>
                  {benefits.map(b => <td key={b.level} className="p-3 text-white">3天</td>)}
                </tr>
                <tr className="hover:bg-white/5 transition-colors">
                  <td className="p-3 text-slate-400">抽奖次数</td>
                  {benefits.map(b => <td key={b.level} className="p-3 text-red-400 font-bold">{b.features[0].split(' ')[1]}</td>)}
                </tr>
                <tr className="hover:bg-white/5 transition-colors">
                  <td className="p-3 text-slate-400">好友动力</td>
                  {benefits.map(b => <td key={b.level} className="p-3 text-green-400 font-bold">{b.features[1].split(' ')[1]}</td>)}
                </tr>
              </tbody>
            </table>
          </div>
